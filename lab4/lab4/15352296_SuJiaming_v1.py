...
def cal_entropy(data_set):
	...
	for key in label_cnt:
		...
		entropy -= posb*math.log(posb,2)
	return entropy

# calculate the gini index of a feature
def cal_gini(data_set,feat,value):
	#count the number of entry of the data set
	num_entry = len(data_set)
	...
	if method.lower() == "gini":
		...
		for i in range(num_feat):
			...
			for value in uniq_val:
				...
				# print(posb,'*',cal_gini(sub_data_set,i,value))
				new_gini += posb*(cal_gini(sub_data_set,i,value))
		...
		for i in range(num_feat):
			...
			for value in uniq_val:
				...
				new_entropy = 0.0
				posb = float(len(sub_data_set))/len(data_set)
				new_entropy += posb*cal_entropy(sub_data_set)
...
# train_set = [[1,1,1,0],[2,1,1,0],[3,2,1,1],[4,2,3,1]]
